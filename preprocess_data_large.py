...
def process_data(p=None):
    ...
    eigen_adj1 = power_adj_list[0].tocsr()
    eigen_adj2 = power_adj_list[1].tocsr()
    x = normalize(data.x, dim=1)

    print('coarsening graph...')
    G = graphs.Graph(adj+adj.T)
    C, Gc, _, _ = coarsen(G, K=10, r=0.99, method='algebraic_JC')
    ...
    for id in tqdm(range(data.y.shape[0])):
        ...
        s = np.maximum(s, 0)
        if p is not None:
            s1 = eigen_adj1[id].toarray()[0]
            s2 = eigen_adj2[id].toarray()[0]
            s3 = np.array(torch.matmul(x[id:id+1], x.transpose(1, 0)))[0]
            s1[id] = 0
            s2[id] = 0
            s3[id] = 0
            s1 = np.maximum(s1, 0)
        ...
        for _ in range(Samples):
            ...
            label = torch.cat([data.y[node_feature_id], torch.zeros((len(super_node_list), 1))]).long()
...
def node_sampling(p=None):
    ...
    for id in range(data_y.shape[0]):
        ...
        for _ in range(Samples):
            ...
            node_feature_id = torch.cat([torch.tensor([id, ]), torch.tensor(sample_index1, dtype=int),
                                         torch.tensor(top_neighbor_index[: k1 + k2 - sample_num2 - sample_num1],
                                                      dtype=int)])
